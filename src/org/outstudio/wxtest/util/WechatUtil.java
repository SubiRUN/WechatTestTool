...
public class WechatUtil {
	 ...
	 * @throws IOException
	 */
	public static String sendLocationEvent(String token, String url,
			double latitude, double longitude, double precision)
			throws IOException {
		String realUrl = buildUrl(token, url);
		String xmlData = createLocationEvent(latitude, longitude, precision)
				.toXml();
	 ...
	 * @throws IOException
	 */
	public static String sendXml(String token, String url, String xml)
			throws IOException {
		String realUrl = buildUrl(token, url);

		String resp = getConn(realUrl).rawData(xml).execute().body();
	...
	public static String getPrettyString(BaseMsg msg) {
		...
		sb.append("CreateTime：").append(createTime).append("（")
				.append(getDatetimeFromCreateTime(createTime)).append("）\n");

		return sb.toString();
	}

	/**
	 * 将微信消息的CreateTime解析成格式化的日期时间字符串
	 */
	...
	private static MenuEvent createMenuClickEvent(String eventKey) {
		...
		event.setEvent(EventType.CLICK);
		buildReqBody(event);
		return event;
	}

	private static LocationEvent createLocationEvent(double latitude,
			double longitude, double precision) {
		LocationEvent event = new LocationEvent(latitude, longitude, precision);
		event.setEvent(EventType.LOCATION);
		buildReqBody(event);
		return event;
	}
